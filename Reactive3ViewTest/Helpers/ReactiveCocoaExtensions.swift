...
struct AssociationKey {
    ...
    //nik
    static var backgroundColor: UInt8 = 14
}
...
extension UIViewController {
    func rac_viewWillDissappear() -> SignalProducer<(), NoError> {
        return self.rac_signalForSelector(#selector(UIViewController.viewWillDisappear(_:)))
            .toSignalProducer()
            .map { _ in () }
            .flatMapError { _ in SignalProducer<(), NoError>.empty  }
    }
...
extension UISlider {
    public var rac_value: MutableProperty<Float> {
        return lazyAssociatedProperty(self, key: &AssociationKey.sliderValue) {
            ...
            self.addTarget(self, action: #selector(UISlider.changed), forControlEvents: UIControlEvents.ValueChanged)
